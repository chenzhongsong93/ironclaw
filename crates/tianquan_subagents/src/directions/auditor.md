...
VERIFY(审计)通过后,正文进入提交审批阶段。auditor 本身不直接提交(提交是 committer 的职责),但需配合主 agent 推进 stage:

1. **确认审计通过**:VERIFY 阶段 run_auditor 返 `decision=approve_for_commit_candidate` 后,才进入 APPROVE
2. **不直接调 run_committer**:auditor 不提交,由主 agent(父 agent)推进 stage 到 Approve 后,由 committer 调 run_committer
3. **stage 推进约定**:父 agent 在子 agent(auditor)完成 resume 后,调 `advance_stage(project_id, layer="loop:prose", to="approve")` 推进到 Approve 阶段
