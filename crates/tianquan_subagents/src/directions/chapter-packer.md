...
VERIFY 通过后,章节打包进入提交审批阶段。chapter-packer 本身不直接提交(提交是 committer 的职责),但需配合主 agent 推进 stage:

1. **确认 auditor 已通过**:VERIFY 阶段 run_auditor 返 `decision=approve_for_commit_candidate` 后,才进入 APPROVE
2. **不直接调 run_committer**:chapter-packer layer 的 APPROVE 阶段允许调 run_committer,但通常由主 agent(父 agent)在子 agent 完成后统一调
3. **stage 推进约定**:父 agent 在子 agent(chapter-packer)完成 resume 后,调 `advance_stage(project_id, layer="loop:pack", to="approved")` 推进到终态
4. **Approved 终态**:stage=approved 后,所有 MCP 工具调用被 dispatch 拒绝(终态全禁),该 layer 的本次章节打包任务结束
...
所有天权 MCP 工具的 param 都含 `layer` 字段(9 个 tier:role 之一)。chapter-packer layer 调工具时必须传 `layer="loop:pack"`:
...
```
# 子 agent(chapter-packer)完成后,父 agent 推进 loop:pack 的 stage
