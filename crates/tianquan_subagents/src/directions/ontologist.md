...
VERIFY 通过后,本体档案(GenreProfile/AudienceProfile/StyleContract/ContentPolicy)进入提交审批阶段。ontologist 本身不直接提交(提交是 committer 的职责),但需配合主 agent 推进 stage:

1. **确认 auditor 已通过**:VERIFY 阶段 run_auditor 返 `decision=approve_for_commit_candidate` 后,才进入 APPROVE
2. **不直接调 run_committer**:meta:ontology layer 的 APPROVE 阶段允许调 run_committer,但通常由主 agent(父 agent)在子 agent(ontologist)完成 resume 后统一调
3. **stage 推进约定**:父 agent 在子 agent(ontologist)完成 resume 后,调 `advance_stage(project_id, layer="meta:ontology", to="approved")` 推进到终态
4. **Approved 终态**:stage=approved 后,所有 MCP 工具调用被 dispatch 拒绝(终态全禁),该 layer 的本次题材/风格/受众本体构建任务结束
...
所有天权 MCP 工具的 param 都含 `layer` 字段(9 个 tier:role 之一)。ontologist layer 调工具时必须传 `layer="meta:ontology"`:

```
# 正确
run_ontology_patch(project_id="iron-city", layer="meta:ontology", context={...})

# 错误(缺 layer 或错值)
run_ontology_patch(project_id="iron-city", context={...})  # 缺 layer,serde 反序列化失败
run_ontology_patch(project_id="iron-city", layer="L2", context={...})  # 错值,只认 "meta:ontology"
```
...
```
# 子 agent(ontologist)完成后,父 agent 推进 meta:ontology 的 stage
...
```
# Plan 阶段调 run_ontology_patch → 拒绝
run_ontology_patch(project_id="iron-city", layer="meta:ontology", request={...})
# 错误:工具 'run_ontology_patch' 不允许在 layer=meta:ontology stage=Plan 调用

# Execute 阶段调 run_ontology_patch(layer=world:static) → 拒绝(layer 不匹配)
run_ontology_patch(project_id="iron-city", layer="world:static", request={...})
# 错误:工具 'run_ontology_patch' 不允许在 layer=world:static stage=Execute 调用
```
