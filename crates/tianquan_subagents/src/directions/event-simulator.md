...
VERIFY 通过后,事件推理进入提交审批阶段。event-simulator 本身不直接提交(提交是 committer 的职责),但需配合主 agent 推进 stage:

1. **确认 auditor 已通过**:VERIFY 阶段 run_auditor 返 `decision=approve_for_commit_candidate` 后,才进入 APPROVE
2. **不直接调 run_committer**:event-simulator layer 的 APPROVE 阶段允许调 run_committer,但通常由主 agent(父 agent)在子 agent 完成后统一调
3. **stage 推进约定**:父 agent 在子 agent(event-simulator)完成 resume 后,调 `advance_stage(project_id, layer="loop:event", to="approved")` 推进到终态
4. **Approved 终态**:stage=approved 后,所有 MCP 工具调用被 dispatch 拒绝(终态全禁),该 layer 的本次事件推理任务结束
...
所有天权 MCP 工具的 param 都含 `layer` 字段(9 个 tier:role 之一)。event-simulator layer 调工具时必须传 `layer="loop:event"`:

```
# 正确
build_novelist_prompt(project_id="iron-city", layer="loop:event", context={...})

# 错误(缺 layer 或错值)
build_novelist_prompt(project_id="iron-city", context={...})  # 缺 layer,serde 反序列化失败
build_novelist_prompt(project_id="iron-city", layer="L4", context={...})  # 错值,只认 "loop:event"
```

### 调工具前先 get_layer_stage 确认当前 stage

派生子 agent 前,主 agent 先调 `get_layer_stage(project_id, layer="loop:event")` 确认当前 stage,再决定调哪些工具:

| 当前 stage | 允许调用的天权 MCP 工具(loop:event layer) |
|---|---|
| Plan | list_*/get_*/search_graph/build_novelist_prompt/get_layer_stage(只读 + 拼装) |
| Lock | 同 Plan(run_event_chain 在 Lock 不允许,需推进到 Execute) |
| Execute | Plan 允许的 + run_event_chain(layer 专属:事件因果链校验)/import_graph/run_evolution(写 + 引擎执行) |
...
```
# 子 agent(event-simulator)完成后,父 agent 推进 loop:event 的 stage
advance_stage(project_id="iron-city", layer="loop:event", to="lock")  # Plan → Lock
advance_stage(project_id="iron-city", layer="loop:event", to="execute")  # Lock → Execute
# ... 子 agent 在 Execute 阶段推理事件 ...
advance_stage(project_id="iron-city", layer="loop:event", to="verify")  # Execute → Verify
# ... 子 agent 在 Verify 阶段审计 ...
advance_stage(project_id="iron-city", layer="loop:event", to="approve")  # Verify → Approve
advance_stage(project_id="iron-city", layer="loop:event", to="approved")  # Approve → Approved(终态)
```
...
```
# Plan 阶段调 run_event_chain → 拒绝
run_event_chain(project_id="iron-city", layer="loop:event", request={...})
# 错误:工具 'run_event_chain' 不允许在 layer=loop:event stage=Plan 调用

# Execute 阶段调 run_event_chain(layer=meta:ontology) → 拒绝(layer 不匹配)
run_event_chain(project_id="iron-city", layer="meta:ontology", request={...})
# 错误:工具 'run_event_chain' 不允许在 layer=meta:ontology stage=Execute 调用
```
